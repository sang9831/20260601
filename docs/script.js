...
function bindElements() {
  ...
  els.commonCount = document.querySelector("#commonCount");
  els.rareCount = document.querySelector("#rareCount");
  els.epicCount = document.querySelector("#epicCount");
  els.legendaryCount = document.querySelector("#legendaryCount");
...
function loadState() {
  try {
    ...
    state.collectedIds = Array.isArray(saved.collectedIds)
      ? saved.collectedIds
      : [];
    state.recent = Array.isArray(saved.recent) ? saved.recent.slice(0, 10) : [];
    state.rarityCounts = {
      common: Number(saved.rarityCounts?.common) || 0,
      rare: Number(saved.rarityCounts?.rare) || 0,
      epic: Number(saved.rarityCounts?.epic) || 0,
      legendary: Number(saved.rarityCounts?.legendary) || 0,
    };
...
async function handleGacha() {
  ...
  try {
    ...
    await preloadImage(pokemon.image);
    ...
    console.error(error);
    els.statusText.textContent = "포켓몬 데이터를 불러오지 못했습니다.";
  } finally {
    window.setTimeout(() => {
      els.stage.classList.remove(
        "is-opening",
...
async function fetchPokemonDetail(id, rarity) {
  ...
  const koName = species.names?.find(
...
function updateHistory(pokemon) {
  state.totalDraws += 1;
  state.rarityCounts[pokemon.rarity.key] += 1;
...
function renderRecent() {
  if (!state.recent.length) {
    els.recentList.innerHTML =
      '<li class="empty-state">아직 획득한 띠부씰이 없습니다.</li>';
    return;
  }
...
function renderStats() {
  ...
  const totalPokemon = state.totalPokemon || TOTAL_POKEMON;
  ...
  els.commonCount.textContent = `${state.rarityCounts.common}회`;
  els.rareCount.textContent = `${state.rarityCounts.rare}회`;
  els.epicCount.textContent = `${state.rarityCounts.epic}회`;
  els.legendaryCount.textContent = `${state.rarityCounts.legendary}회`;
}
...
function resetStickerCard() {
  els.stickerCard.hidden = true;
  els.stickerCard.classList.remove("is-visible");
  els.stickerCard.removeAttribute("data-rarity");
  els.pokemonImage.removeAttribute("src");
  els.pokemonImage.alt = "";
  els.stickerNumber.textContent = "#000";
  els.stickerRarity.textContent = "COMMON";
  els.pokemonName.textContent = "Pokemon";
  els.pokemonTypes.textContent = "Type: -";
  els.pokemonHeight.textContent = "Height: -";
  els.pokemonWeight.textContent = "Weight: -";
}
